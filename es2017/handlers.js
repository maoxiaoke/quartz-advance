 ...
 */ export async function handleCreate(argv) {
    var _argv_strategy, _argv_links;
    console.log();
    intro(chalk.bgGreen.black(` Quartz v${version} `));
    const contentFolder = path.join(cwd, argv.directory);
    let setupStrategy = (_argv_strategy = argv.strategy) === null || _argv_strategy === void 0 ? void 0 : _argv_strategy.toLowerCase();
    let linkResolutionStrategy = (_argv_links = argv.links) === null || _argv_links === void 0 ? void 0 : _argv_links.toLowerCase();
    ...
    if (setupStrategy && linkResolutionStrategy) {
        ...
        if (setupStrategy !== "new") {
            // Error handling
            if (!sourceDirectory) {
                outro(chalk.red(`Setup strategies (arg '${chalk.yellow(`-${CreateArgv.strategy.alias[0]}`)}') other than '${chalk.yellow("new")}' require content folder argument ('${chalk.yellow(`-${CreateArgv.source.alias[0]}`)}') to be set`));
                process.exit(1);
            } else {
                if (!fs.existsSync(sourceDirectory)) {
                    outro(chalk.red(`Input directory to copy/symlink 'content' from not found ('${chalk.yellow(sourceDirectory)}', invalid argument "${chalk.yellow(`-${CreateArgv.source.alias[0]}`)})`));
                    process.exit(1);
                } else if (!fs.lstatSync(sourceDirectory).isDirectory()) {
                    outro(chalk.red(`Source directory to copy/symlink 'content' from is not a directory (found file at '${chalk.yellow(sourceDirectory)}', invalid argument ${chalk.yellow(`-${CreateArgv.source.alias[0]}`)}")`));
                    process.exit(1);
                }
            }
        }
    ...
    if (!setupStrategy) {
        setupStrategy = exitIfCancel(await select({
    ...
    if (setupStrategy === "copy" || setupStrategy === "symlink") {
        ...
        if (!sourceDirectory) {
            originalFolder = escapePath(exitIfCancel(await text({
...
`);
    ...
    if (!linkResolutionStrategy) {
        ...
        linkResolutionStrategy = exitIfCancel(await select({
 ...
 */ export async function handleBuild(argv) {
    ...
    if (argv.serve) {
        ...
        const server = http.createServer(async (req, res)=>{
            ...
            var _req_url_split_;
            let fp = (_req_url_split_ = (_req_url2 = req.url) === null || _req_url2 === void 0 ? void 0 : _req_url2.split("?")[0]) !== null && _req_url_split_ !== void 0 ? _req_url_split_ : "/";
            // handle redirects
            if (fp.endsWith("/")) {
                // /trailing/
 ...
 */ export async function handleSync(argv) {
    ...
    if (argv.commit) {
        ...
        var _argv_message;
        const commitMessage = (_argv_message = argv.message) !== null && _argv_message !== void 0 ? _argv_message : `Quartz sync: ${currentTimestamp}`;
        spawnSync("git", [
            "add",
            "."
        ], {
